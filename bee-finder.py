...
LANGUAGE_CHARACTER_TRANSFORMATIONS = {
    "de": {"ä": "ae", "ü": "ue", "ö": "oe", "ß": "ss"}
}
...
def apply_character_transformations(s: str, transformations):
    res = s
    for orig in transformations:
        print(orig)
        print(transformations[orig])
        res = res.replace(orig, transformations[orig])
    return res
